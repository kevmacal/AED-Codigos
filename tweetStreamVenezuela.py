...
file_raw = open('./data/dataStreamVenezuela_' + time.strftime('%Y%m%d') + '.json', 'a')
...
class StdOutListener(StreamListener):
    ...
    def on_data(self, data):
        ...
        if tweet['coordinates']:
            ...
            print (("Tweet Venezuela Coordinates:" + str(tweet['coordinates']) + "Numero: " + str(self.contTweets)))
        return True
...
#    stream.filter(locations=[-124.482009887695, 32.5295219421387, -114.13077545166, 42.0095024108887,]) California
    stream.filter(locations=[-72.398267, 8.240802, -65.016530, 11.061245])  # Venezuela Poblacion
